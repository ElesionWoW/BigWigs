...
BigWigsBars.consoleOptions = {
	...
	args = {
		...
		intervalbar = {
			...
			order = 12,
			get = function()
				return BigWigsBars.db.profile.intervalbar
			end,
			set = function(v)
				BigWigsBars.db.profile.intervalbar = v
			end,
		},
		growup = {
			type = "toggle",
			...
			order = 13,
			get = function()
				return BigWigsBars.db.profile.growup
			end,
			set = function(v)
		...
		reverse = {
			...
			order = 14,
			get = function()
				return BigWigsBars.db.profile.reverse
			end,
			set = function(v)
				BigWigsBars.db.profile.reverse = v
			end,
			--passValue = "reverse",
		},
		scale = {
			...
			step = 0.1,
			get = function()
				return BigWigsBars.db.profile.scale
			end,
			set = function(v)
				BigWigsBars.db.profile.scale = v
			end,
		},
		texture = {
			type = "text",
			...
			order = 16,
			get = function()
				return BigWigsBars.db.profile.texture
			end,
			set = function(v)
				BigWigsBars.db.profile.texture = v
			end,
		...
		emphasize = {
			...
			args = {
				emphasize = {
					...
					get = function()
						return BigWigsBars.db.profile.emphasize
					end,
					set = function(v)
						BigWigsBars.db.profile.emphasize = v
					end,
					order = 1,
				},
				growup = {
					...
					order = 2,
					get = function()
						return BigWigsBars.db.profile.emphasizeGrowup
					end,
					set = function(v)
				...
				flash = {
					...
					get = function()
						return BigWigsBars.db.profile.emphasizeFlash
					end,
					set = function(v)
						BigWigsBars.db.profile.emphasizeFlash = v
					end,
					disabled = shouldDisableEmphasizeOption,
					order = 3,
				},
				move = {
					...
					get = function()
						return BigWigsBars.db.profile.emphasizeMove
					end,
					set = function(v)
						BigWigsBars.db.profile.emphasizeMove = v
					end,
					disabled = shouldDisableEmphasizeOption,
					order = 4,
				},
				scale = {
					...
					step = 0.1,
					get = function()
						return BigWigsBars.db.profile.emphasizeScale
					end,
					set = function(v)
						BigWigsBars.db.profile.emphasizeScale = v
					end,
					disabled = function()
						if not BigWigsBars.db.profile.emphasizeMove then
							return true
						end
...
function BigWigsBars:BigWigs_StartBar(module, text, time, icon, otherc, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, emphasize, target, spell)
	...
	if BigWigsColors and type(BigWigsColors) == "table" then
		if type(otherc) ~= "boolean" or not otherc then
			c1, c2, c3, c4, c5, c6, c7, c8, c9, c10 = BigWigsColors:BarColor(time)
		end
	end
	...
	if self.frames.emphasizeAnchor and self.db.profile.emphasize and emphasize == true and (self.db.profile.emphasizeMove or self.db.profile.emphasizeFlash) then
		...
		if time > 20 then
			...
			if self.db.profile.emphasizeMove then
				if not self.frames.emphasizeAnchor.emphasizeTimers[module] then
					self.frames.emphasizeAnchor.emphasizeTimers[module] = {}
				end
				if self.frames.emphasizeAnchor.emphasizeTimers[module][id] then
			...
			if self.db.profile.emphasizeFlash then
				if not self.frames.emphasizeAnchor.flashTimers[module] then
					self.frames.emphasizeAnchor.flashTimers[module] = {}
				end
				if self.frames.emphasizeAnchor.flashTimers[module][id] then
...
function BigWigsBars:FlashBar(module, id)
	if not flashColors then
		generateColors()
	end
	if self.frames.emphasizeAnchor and self.frames.emphasizeAnchor.flashTimers[module] then
		self.frames.emphasizeAnchor.flashTimers[module][id] = nil
	end
...
function BigWigsBars:SetScale(msg, supressreport)
	...
	if scale and scale >= minscale and scale <= maxscale then
		...
		if not supressreport then
			self.core:Print(L["Scale is set to %s"], scale)
		end
	end
end
...
function BigWigsBars:ToggleUp(supressreport)
	...
	if not supressreport then
		self.core:Print(L["Bars now grow %s"], (t and L["Up"] or L["Down"]))
	end
end
...
function BigWigsBars:SetupFrames(emphasize)
	...
	if self.db.profile.emphasize and self.frames.emphasizeAnchor then
		return
	end
